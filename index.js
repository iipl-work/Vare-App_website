...
var token ="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJwaG9uZSI6IjkxOTcwMzIyMDk3NCIsInVzZXJJZCI6ImQzNmNlZDIwLTFkYjUtMTFlYy1hZmIwLWMzYzkzYzc4ZThmMyIsInZlbmRvcklkIjoiZTI3YWFmOTAtMjBlYS0xMWVjLWFjNzQtODkzMWZiOWIzMDYyIiwiaWF0IjoxNjM2NDQyMTMwLCJleHAiOjMzMTk0MDQyMTMwfQ.llmnWVk8YAg7Q9YZW3rQKs_DAhi0hZaUAsQ7sQrSDWY";
app.get("/", (req, res) => {
  var menu = [];
...
var config = {
  method: 'post',
  url: 'https://k6u06d3vrf.execute-api.ap-south-1.amazonaws.com/dev/api/product?pageNo='+pageNo,
  headers: { 
    'x-user-auth': token,
...
app.get("/terms-conditions", async (req, res) => {
  ...
  var config = {
    method: 'get',
    url: 'http://k6u06d3vrf.execute-api.ap-south-1.amazonaws.com/dev/api/staticpages?pageName=terms&conditions',
    headers: { }
  };
  ...
  var response=await axios(config)
    ...
    console.log(JSON.stringify(response.data));
  res.render("terms&conditions", {result:response.data.data,menu:menu });
});
...
var config = {
  method: 'post',
  url: 'https://k6u06d3vrf.execute-api.ap-south-1.amazonaws.com/dev/api/blog?pageNo='+pageNo,
  headers: { 
    'x-user-auth': token,
    'Content-Type': 'application/json'
  },
